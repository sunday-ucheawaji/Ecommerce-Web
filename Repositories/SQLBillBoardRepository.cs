...
namespace EcommerceWeb.Repositories
{
    public class SQLBillBoardRepository:IBillBoardRepository
    {
        ...
        {
            return await _dbContext.BillBoards.Include(pp=> pp.ProductImages).ToListAsync();
        }

        public async Task<BillBoard?> GetByIdAsync(Guid id)
        {
            return await _dbContext.BillBoards.Include(pp => pp.ProductImages).FirstOrDefaultAsync(x => x.BillBoardId == id);
        }
        ...
        {
            var existingBillBoard = await _dbContext.BillBoards.FirstOrDefaultAsync(x => x.BillBoardId == id);
